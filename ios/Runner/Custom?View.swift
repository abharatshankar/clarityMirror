...
//        resultsControl?.selectSwitch.addTarget(self, action: #selector(changeResultControl), for: .valueChanged)
    ...
    private func handleMethodCall(call: FlutterMethodCall, result: FlutterResult, imgStr: FlutterStandardTypedData) {
            if call.method == "methodName" {
                ...
                // Handle the method call, call Swift methods or return values
                // Example:
                print("here--------")
                let argument = call.arguments as! String
                self.handleMethodName(argument: argument, result: result)
            } else {
...
//        result(notification.object ?? "")
        ...
        private func handleMethodName(argument: String, result: FlutterResult) {
            // Your Swift code here
            // Example:
            print("Received method call with argument: \(argument)")
            // Perform any Swift operation here and return the result
            result("nil value") // You can return a result if needed
        }
...
//        uploadImage(config: self.config, serviceMager: serviceManager!, imageData: self.capturedImage!,tagsToAnalysed:tagsToAnalysed)
    ...
    func convertUIImageToByteArray(image: UIImage) -> FlutterStandardTypedData? {
    ...
    func getImagePath(for image: UIImage) -> String? {
        ...
        do {
            ...
            let fileURL = temporaryDirectoryURL.appendingPathComponent(UUID().uuidString + ".png")
...
extension ViewController : WidgetScreenCallBacks {
    ...
    func widgetCaptureSucess(isSucess:Bool,image:UIImage){
        ...
        if let appDelegate = UIApplication.shared.delegate as? AppDelegate {
              // Call the method from AppDelegate
            appDelegate.callFlutterFunction(message: self.getImagePath(for: image)!)
            }
        return
    }
            ...
            if serviceResponse.status {
                do{
                    ...
                    if tagJsonResult.ErrorCode == nil{
                        let analysisResult = addJSONResultToArray(widgetResult : tagJsonResult.Tags,_config : self.config)
                        DispatchQueue.main.async {
